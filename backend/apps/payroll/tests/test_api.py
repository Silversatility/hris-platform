...
from apps.payroll.xendit_client import XenditError
...
def test_pay_via_xendit_requires_agent_bank_details(staff_user, agent, pay_run):
    payout = CommissionPayout.objects.create(pay_run=pay_run, agent=agent)
    pay_run.status = PayRun.Status.COMPLETED
    pay_run.save(update_fields=["status"])
    client = APIClient()
    ...
    response = client.post(reverse("commission-payout-pay-via-xendit", args=[payout.id]))

    assert response.status_code == 400
    assert "bank" in str(response.json()).lower()


def test_pay_via_xendit_requires_completed_pay_run(staff_user, agent, pay_run):
    agent.bank_code = "PH_GCASH"
    agent.bank_account_number = "09171234567"
    agent.save(update_fields=["bank_code", "bank_account_number"])
    payout = CommissionPayout.objects.create(pay_run=pay_run, agent=agent)
    client = APIClient()
    client.force_authenticate(staff_user)
...
def test_pay_via_xendit_success(staff_user, agent, pay_run):
    agent.bank_code = "PH_GCASH"
    agent.bank_account_number = "09171234567"
    agent.bank_account_holder_name = "Carlo Reyes"
    agent.save(update_fields=["bank_code", "bank_account_number", "bank_account_holder_name"])
    payout = CommissionPayout.objects.create(pay_run=pay_run, agent=agent)
    pay_run.status = PayRun.Status.COMPLETED
    pay_run.save(update_fields=["status"])
    client = APIClient()
    ...
    with patch(
        "apps.payroll.views.xendit_client.create_disbursement",
        return_value={"id": "disb-abc123", "status": "PENDING"},
    ) as mock_create:
        response = client.post(reverse("commission-payout-pay-via-xendit", args=[payout.id]))
    ...
    assert data["is_paid"] is True
    assert data["payment_method"] == "gcash"
    assert data["payment_reference"] == "disb-abc123"
    mock_create.assert_called_once()
    call_kwargs = mock_create.call_args.kwargs
    assert call_kwargs["channel_code"] == "PH_GCASH"
    assert call_kwargs["account_number"] == "09171234567"
    assert call_kwargs["account_name"] == "Carlo Reyes"

    payout.refresh_from_db()
    assert payout.is_paid is True
    assert payout.payment_reference == "disb-abc123"


def test_pay_via_xendit_surfaces_xendit_error(staff_user, agent, pay_run):
    agent.bank_code = "PH_GCASH"
    agent.bank_account_number = "09171234567"
    agent.save(update_fields=["bank_code", "bank_account_number"])
    payout = CommissionPayout.objects.create(pay_run=pay_run, agent=agent)
    ...
    with patch(
        "apps.payroll.views.xendit_client.create_disbursement",
        side_effect=XenditError("Invalid channel_code for this account."),
    ):
        response = client.post(reverse("commission-payout-pay-via-xendit", args=[payout.id]))

    assert response.status_code == 400
    assert "Invalid channel_code" in str(response.json())

    payout.refresh_from_db()
    assert payout.is_paid is False


def test_pay_via_xendit_cannot_pay_twice(staff_user, agent, pay_run):
    agent.bank_code = "PH_GCASH"
    agent.bank_account_number = "09171234567"
    agent.save(update_fields=["bank_code", "bank_account_number"])
...
def test_non_staff_cannot_pay_via_xendit(employee, agent, pay_run):
    agent.bank_code = "PH_GCASH"
    agent.bank_account_number = "09171234567"
    agent.save(update_fields=["bank_code", "bank_account_number"])
    payout = CommissionPayout.objects.create(pay_run=pay_run, agent=agent)
